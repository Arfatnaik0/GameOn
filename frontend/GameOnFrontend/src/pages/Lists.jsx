  ...
  const renderCard = (game, config) => (
    ...
    >
      ...
      <div style={{ padding: '12px 14px' }}>
        <p style={{ fontFamily: 'Rajdhani, sans-serif', fontWeight: 700, fontSize: 14, color: '#fff', marginBottom: 10, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {game.gameName}
        </p>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0608', color: '#fff', fontFamily: 'Outfit, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: isMobile ? '20px 16px' : '40px' }}>
        {isLoading ? (
          <div style={{ display: 'flex', gap: 16, flexDirection: isMobileLayout ? 'column' : 'row' }}>
            {[1, 2, 3].map(i => (
              <div key={i} style={{ flex: 1, height: 300, borderRadius: 20, background: 'rgba(255,255,255,0.04)' }} />
            ))}
          </div>
        ) : entries.length === 0 ? (
              ...
              {Object.entries(STATUS_CONFIG).map(([status, config]) => (
                <button
                  ...
                  style={{
                    ...
                    flexShrink: 0,
                    background: activeTab === status ? config.bg : 'rgba(255,255,255,0.04)',
                    border: `1px solid ${activeTab === status ? config.border : 'rgba(255,255,255,0.08)'}`,
                    color: activeTab === status ? config.color : 'rgba(255,255,255,0.4)',
                    fontSize: 13, fontWeight: 600, transition: 'all 0.2s',
                  }}>
                  <div style={{ width: 7, height: 7, borderRadius: '50%', background: config.color }} />
                  {config.label}
                  <span style={{ fontSize: 11, opacity: 0.7 }}>({byStatus(status).length})</span>
                </button>
              ))}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: isTablet ? 'repeat(2, 1fr)' : '1fr', gap: 12 }}>
              {byStatus(activeTab).length === 0 ? (
                <div style={{ padding: '32px 16px', borderRadius: 16, border: `1px dashed ${STATUS_CONFIG[activeTab].border}`, textAlign: 'center', gridColumn: '1 / -1' }}>
                  <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)' }}>No games here yet</p>
                </div>
              ) : (
                byStatus(activeTab).map(game => renderCard(game, STATUS_CONFIG[activeTab]))
              )}
            </div>
          </>
        ) : (
              ...
              return (
                <div key={status} style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                    ...
                    <h2 style={{ fontFamily: 'Rajdhani, sans-serif', fontWeight: 700, fontSize: 17, color: '#fff' }}>{config.label}</h2>
                    <span style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: config.bg, border: `1px solid ${config.border}`, color: config.color }}>
                      {games.length}
                    </span>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    ) : games.map(game => renderCard(game, config))}
                  </div>
                </div>
              )
            })}
